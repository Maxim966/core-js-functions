...
function getArgumentsCount(funcs) {
  return funcs.map((item) => {
    function countLength(func) {
      return func.length;
    }
    return countLength(item);
  });
}
 ...
 *   getPolynom()      => null
 */
function getPolynom(...parameters) {
  if (parameters.length === 0) {
    return null;
  }

  const degree = parameters.length - 1;

  return function countValue(x) {
    let result = 0;

    for (let index = 0; index <= degree; index += 1) {
      result += parameters[index] * x ** (degree - index);
    }
...
function partialUsingArguments(fn, ...args1) {
  return function connectionOfArguments(...arg) {
    return fn.apply(this, [...args1, ...arg]);
  };
}
